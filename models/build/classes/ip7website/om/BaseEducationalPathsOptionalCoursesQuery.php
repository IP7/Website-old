...
abstract class BaseEducationalPathsOptionalCoursesQuery extends ModelCriteria
{
    ...
    public function filterByCourseId($courseId = null, $comparison = null)
    {
        if (is_array($courseId)) {
            ...
            if (isset($courseId['min'])) {
                $this->addUsingAlias(EducationalPathsOptionalCoursesPeer::COURSE_ID, $courseId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($courseId['max'])) {
                $this->addUsingAlias(EducationalPathsOptionalCoursesPeer::COURSE_ID, $courseId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    public function filterByPathId($pathId = null, $comparison = null)
    {
        if (is_array($pathId)) {
            $useMinMax = false;
            if (isset($pathId['min'])) {
                $this->addUsingAlias(EducationalPathsOptionalCoursesPeer::PATH_ID, $pathId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($pathId['max'])) {
                $this->addUsingAlias(EducationalPathsOptionalCoursesPeer::PATH_ID, $pathId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
